...
Be precise and match the expected format exactly.`,
  ...
  async askQuestion(question, options = []) {
    ...
    if (!settings.enabled || !settings.apiKey) {
      console.log("[SBS LLM] Not configured or disabled");
      return null;
    }

    const provider = this.PROVIDERS[settings.provider];
    if (!provider) {
      console.error("[SBS LLM] Unknown provider:", settings.provider);
      return null;
    }
    ...
    try {
      ...
      if (response) {
        console.log("[SBS LLM] Got answer:", response);
        return response.trim();
      }

      return null;
    } catch (error) {
      console.error("[SBS LLM] Error:", error);
      return null;
    }
  },
  ...
  async callOpenAICompatible(provider, apiKey, userPrompt) {
    ...
    if (!response.ok) {
      ...
      console.error("[SBS LLM] API error:", response.status, error);
      return null;
    }
  ...
  async callAnthropic(provider, apiKey, userPrompt) {
    ...
    if (!response.ok) {
      const error = await response.text();
      console.error("[SBS LLM] Anthropic API error:", response.status, error);
      return null;
    }
